## gns3util cluster-control jobs run

Trigger a background job on a specific node

```
gns3util cluster-control jobs run [job_name] [flags]
```
...
```
  -h, --help             help for run
      --node-id string   Node ID to run the job on (required)
```
