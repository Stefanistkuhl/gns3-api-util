## gns3util template new

Create a template

```
gns3util template new [flags]
```

### Examples

```
gns3util -s https://controller:3080 create template -n some_name -t vpcs
```
...
```
      ...
      --compute-id string            Compute ID
  -f, --default-name-format string   Default name format
  -h, --help                         help for new
  -n, --name string                  Desired name of the template
      --symbol string                Symbol name
  -d, --template-id string           Desired ID for template, leave empty to use a generated one
  -t, --template-type string         Template type
      --usage string                 Usage
