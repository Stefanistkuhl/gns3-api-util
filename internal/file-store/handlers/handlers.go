...
func (f *FilestoreHandlers) HandleInitUpload(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		WriteApiError(w, "failed to get claims from jwt", ErrCodeGetClaims, "failed to get claims from jwt even though this is past middleware and shouldn't happen", http.StatusInternalServerError)
		return
	}
	...
	if decodeErr := json.NewDecoder(r.Body).Decode(&req); decodeErr != nil {
		...
		WriteApiError(w, "Invalid request body", ErrCodeInvalidJSON, decodeErr.Error(), http.StatusBadRequest)
		return
	}

	if req.Filename == "" {
		WriteApiError(w, "filename is required", ErrCodeInvalidInput, "missing required field: filename", http.StatusBadRequest)
		return
	}

	if req.SizeBytes <= 0 {
		WriteApiError(w, "size_bytes must be greater than 0", ErrCodeInvalidInput, "invalid value for size_bytes", http.StatusBadRequest)
		return
	}
	...
	if uuidErr != nil {
		...
		WriteApiError(w, "failed to generate a uuid", ErrCodeFailedToGenerateUUID, "somehow failed to generate a uuid for the uploaded file if somehow someone manages to trigger this i am very impressed", http.StatusInternalServerError)
		return
	}
	...
	if insertErr != nil {
		...
		WriteApiError(w, "failed to insert values into db", ErrCodeDBErr, insertErr.Error(), http.StatusInternalServerError)
		return
	}
	...
	writeErr := writeJSON(w, InitUploadResponse{
	...
	if writeErr != nil {
		...
		WriteApiError(w, "failed to write response", ErrCodeInternal, writeErr.Error(), http.StatusInternalServerError)
		return
	}
}
...
func (f *FilestoreHandlers) HandleStreamUpload(w http.ResponseWriter, r *http.Request) {
	...
	if fileUUID == "" {
		WriteApiError(w, "file_uuid is required", ErrCodeInvalidInput, "missing file_uuid in URL path", http.StatusBadRequest)
		return
	}
	claims, ok := middleware.GetClaims(r)
	if !ok {
	...
	if getDbIdErr != nil {
		if errors.Is(getDbIdErr, sql.ErrNoRows) {
			WriteApiError(w, "file not found", ErrCodeFileNotFound, "no file found with the provided uuid", http.StatusNotFound)
			return
		}
		f.Logger.Error("Failed to get owner of file by UUID", "err", getDbIdErr, "file_uuid", fileUUID)
		WriteApiError(w, "failed to query db for file info", ErrCodeDBErr, getDbIdErr.Error(), http.StatusInternalServerError)
		return
	}
	if dbId != userID {
		WriteApiError(w, "forbidden", ErrCodeForbidden, "you do not have permission to upload to this file", http.StatusForbidden)
		return
	}
		...
		if len(parts) == 2 && parts[0] == "bytes" {
			...
			if len(rangeParts) >= 1 {
				...
				if clientStart != offset {
					WriteApiError(w, "upload offset mismatch", ErrCodeOffsetMismatch,
	...
	if offset > 0 {
		...
		if openErr != nil {
			...
			WriteApiError(w, "failed to resume upload", ErrCodeInternal, openErr.Error(), http.StatusInternalServerError)
			return
		}
		...
		if _, resumeCopyErr := io.Copy(hasher, existingFile); resumeCopyErr != nil {
			...
			WriteApiError(w, "failed to hash existing data", ErrCodeInternal, resumeCopyErr.Error(), http.StatusInternalServerError)
			return
		}
		if closeErr := existingFile.Close(); closeErr != nil {
			f.Logger.Error("Failed to close existing file", "err", closeErr)
	...
	if createFileErr != nil {
		...
		WriteApiError(w, "failed to create file on disk", ErrCodeInternal, createFileErr.Error(), http.StatusInternalServerError)
		return
	}
	if _, err := dst.Seek(0, io.SeekEnd); err != nil {
		...
		WriteApiError(w, "failed to seek file", ErrCodeInternal, err.Error(), http.StatusInternalServerError)
		return
	}
	var moved bool
	defer func() {
	...
	if copyErr != nil {
		...
		WriteApiError(w, "upload interrupted", ErrCodeInternal, copyErr.Error(), http.StatusInternalServerError)
		return
	}
	if syncErr := dst.Sync(); syncErr != nil {
		f.Logger.Error("Failed to sync file", "err", syncErr, "file_uuid", fileUUID)
		WriteApiError(w, "failed to sync file", ErrCodeInternal, syncErr.Error(), http.StatusInternalServerError)
		return
	}
	...
	if checkDirErr != nil {
		...
		WriteApiError(w, "failed to create shard directories", ErrCodeInternal, checkDirErr.Error(), http.StatusInternalServerError)
		return
	}
	...
	if renameErr != nil {
		...
		WriteApiError(w, "failed to move file to object store", ErrCodeInternal, renameErr.Error(), http.StatusInternalServerError)
		return
	}
	...
	if finalizeErr != nil {
		...
		WriteApiError(w, "failed to finalize file in database", ErrCodeDBErr, finalizeErr.Error(), http.StatusInternalServerError)
		return
	}
	ret := FinalizeUploadResponse{
		FileUUID:       fileUUID,
		Status:         models.FileStatusAvailable,
		ChecksumSHA256: finalHash,
		SizeBytes:      totalSize,
	}
	writeErr := writeJSON(w, ret)
	if writeErr != nil {
...
func (f *FilestoreHandlers) GetUploadStatus(w http.ResponseWriter, r *http.Request) {
	...
	writeErr := writeJSON(w, GetUploadStatusResponse{
		Offset: offset,
	})
	if writeErr != nil {
		f.Logger.Error("Failed to write response", "err", writeErr, "file_uuid", fileUUID)
