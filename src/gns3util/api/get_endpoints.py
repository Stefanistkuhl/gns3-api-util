...
class GNS3GetAPI(GNS3APIClient):
    ...
    def node_by_id(self, project_id, node_id):
        return self._api_call(
            f"projects/{project_id}/nodes/{node_id}", verify=self.verify
        )

    def node_links_by_id(self, project_id, node_id):
        return self._api_call(
            f"projects/{project_id}/nodes/{node_id}/links", verify=self.verify
        )

    def node_dynamips_auto_idlepc(self, project_id, node_id):
        return self._api_call(
            f"projects/{project_id}/nodes/{node_id}/dynamips/auto_idlepc",
            verify=self.verify,
        )

    def node_dynamips_idlepc_proposals(self, project_id, node_id):
        return self._api_call(
            f"projects/{project_id}/nodes/{node_id}/dynamips/idlepc_proposals",
            verify=self.verify,
        )
    ...
    def link_filters(self, project_id, link_id):
        return self._api_call(
            f"projects/{project_id}/links/{link_id}/available_filters",
            verify=self.verify,
        )

    def link(self, project_id, link_id):
        return self._api_call(
            f"projects/{project_id}/links/{link_id}", verify=self.verify
        )
    ...
    def drawing(self, project_id, drawing_id):
        return self._api_call(
            f"projects/{project_id}/drawings/{drawing_id}", verify=self.verify
        )
    ...
    def compute_by_id_docker_images(self, compute_id):
        return self._api_call(
            f"computes/{compute_id}/docker/images", verify=self.verify
        )

    def compute_by_id_virtualbox_vms(self, compute_id):
        return self._api_call(
            f"computes/{compute_id}/virtualbox/vms", verify=self.verify
        )
    ...
    def download_project_file(self, project_id, file_path):
        ...
        return self._api_call(
            f"projects/{project_id}/files/{encoded_file_path}", verify=self.verify
        )
    ...
    def project_export(self, project_id=str, export_params={}):
        ...
        return self._api_call(
            f"projects/{project_id}/export", stream=True, params=params
        )
