...
from .utils import (
    ...
    fuzzy_delete_class_params,
    fuzzy_delete_class_wrapper,
    fuzzy_delete_exercise_params,
    fuzzy_delete_exercise_wrapper,
)
...
Has data: False
"""
_zero_arg_no_data = {"prune_images": "prune_images"}
    ...
    def make_cmd(func=func, help_option=current_help_option, epilog=epiloge):
        ...
        def cmd_func(ctx: click.Context):
            ...
            execute_and_print(
                ctx,
                api_delete_client,
                lambda client: getattr(api_delete_client, func)(),
            )
    ...
    def make_cmd(func=func, help_option=current_help_option, epilog=epiloge):
        ...
        def cmd_func(ctx: click.Context, arg):
            ...
            execute_and_print(
                ctx,
                api_delete_client,
                lambda client: getattr(api_delete_client, func)(arg),
            )
    ...
    def make_cmd(func=func, help_option=current_help_option, epilog=epiloge):
        ...
        def cmd_func(ctx: click.Context, arg1, arg2):
            ...
            execute_and_print(
                ctx,
                api_delete_client,
                lambda client: getattr(api_delete_client, func)(arg1, arg2),
            )
...
@click.option(
    ...
    default=True,
    is_flag=True,
    help="Require confirmation for deletion",
)
@click.option(
    "-n", "--non_interactive", type=str, help="Run the command non interactively"
)
@click.option("-a", "--delete_all", is_flag=True, help="Delete all classes")
@click.option(
    "-e", "--delete_exercises", is_flag=True, help="Delete all exercies of that class"
)
@click.pass_context
def fuzzy_delete_class(
    ctx: click.Context, multi, confirm, non_interactive, delete_all, delete_exercises
):
...
@click.option(
    ...
    "--select_class",
    is_flag=True,
    help="Set the class from which to delete the exercise",
)
@click.option(
    "-fg",
    "--select_group",
    is_flag=True,
    help="Set the group from which to delete the exercise",
)
@click.option("-a", "--delete_all", is_flag=True, help="Delete all exercises")
@click.pass_context
