...
from gns3util.schemas.compute.qemu_nodes import (
    QemuConsoleType,
    QemuPlatform,
    QemuAdapterType,
    QemuOnCloseAction,
...
class QemuTemplate(TemplateBase):
    ...
    platform: Optional[QemuPlatform] = Field(
        QemuPlatform.x86_64, description="Platform to emulate")
    linked_clone: Optional[bool] = Field(
        True, description="Whether the VM is a linked clone or not")
    ...
    maxcpus: Optional[int] = Field(
        1, ge=1, le=255, description="Maximum number of hotpluggable vCPUs")
    adapters: Optional[int] = Field(
        1, ge=0, le=275, description="Number of adapters")
    adapter_type: Optional[QemuAdapterType] = Field(
        QemuAdapterType.e1000, description="QEMU adapter type")
    ...
    hdc_disk_image: Optional[str] = Field(
    ...
    hdd_disk_image: Optional[str] = Field(
    ...
    kernel_image: Optional[str] = Field(
    ...
    tpm: Optional[bool] = Field(
        False, description="Enable Trusted Platform Module (TPM)")
    ...
    cpu_throttling: Optional[int] = Field(
        0, ge=0, le=800, description="Percentage of CPU allowed for QEMU")
